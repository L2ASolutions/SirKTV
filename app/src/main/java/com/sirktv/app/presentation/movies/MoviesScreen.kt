...
fun MoviesScreen(
    ...
    Column(
        ...
        LazyRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            item {
                CategoryPill(label = "All Movies", selected = uiState.selectedCategoryId == null) {
        ...
        Box(Modifier.padding(top = Dimens.SpaceMd)) {
            if (uiState.visibleMovies.isEmpty()) {
                Text("No movies found in this category.", color = Color.White.copy(alpha = 0.5f), fontSize = 13.sp)
            } else {
                LazyVerticalGrid(
                    ...
                    items(uiState.visibleMovies, key = { it.id }) { movie ->
                        Column {
                            ...
                            FavoriteToggleChip(
                                isFavorite = movie.isFavorite,
                                onToggle = { viewModel.onToggleFavorite(movie.id) },
