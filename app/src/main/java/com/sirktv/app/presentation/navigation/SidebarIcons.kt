 ...
 *
 * Every draw body below is wrapped in try/catch — a real-device report
 * showed the Live TV item rendering as a blank row, which a swallowed
 * exception mid-draw (a degenerate Path op, a NaN from an in-flight color/
 * scale animation, etc.) would produce with no other symptom. On failure
 * each icon falls back to a simple, guaranteed-safe glyph instead of
    ...
    fun Home(tint: Color, modifier: Modifier = Modifier) {
        Canvas(modifier = modifier) {
            try {
                ...
                Log.e("SirKTV_Sidebar", "Home icon draw failed: ${e.stackTraceToString()}")
                drawRoundRect(color = tint, cornerRadius = CornerRadius(3f), size = size)
            }
        }
    }
    ...
    fun LiveTv(tint: Color, modifier: Modifier = Modifier) {
        Canvas(modifier = modifier) {
            try {
                ...
                Log.e("SirKTV_Sidebar", "LiveTv icon draw failed: ${e.stackTraceToString()}")
                drawLiveTvFallback(tint)
            }
        }
    }

    /** Simple, deliberately minimal TV shape — a guaranteed-safe fallback if the primary Live TV draw ever throws. */
    private fun DrawScope.drawLiveTvFallback(color: Color) {
        ...
        drawRoundRect(
            ...
            size = Size(size.width - 4.dp.toPx(), size.height * 0.75f)
        )
        drawLine(
            color = color,
    ...
    fun Movies(tint: Color, modifier: Modifier = Modifier) {
        Canvas(modifier = modifier) {
            try {
                ...
                Log.e("SirKTV_Sidebar", "Movies icon draw failed: ${e.stackTraceToString()}")
                drawRoundRect(color = tint, cornerRadius = CornerRadius(3f), size = size)
            }
        }
    }
    ...
    fun Series(tint: Color, modifier: Modifier = Modifier) {
        Canvas(modifier = modifier) {
            try {
                ...
                Log.e("SirKTV_Sidebar", "Series icon draw failed: ${e.stackTraceToString()}")
                drawRoundRect(color = tint, cornerRadius = CornerRadius(3f), size = size)
            }
        }
    }
    ...
    fun Sports(tint: Color, modifier: Modifier = Modifier) {
        Canvas(modifier = modifier) {
            try {
                ...
                Log.e("SirKTV_Sidebar", "Sports icon draw failed: ${e.stackTraceToString()}")
                drawRoundRect(color = tint, cornerRadius = CornerRadius(3f), size = size)
            }
        }
    }
    ...
    fun Search(tint: Color, modifier: Modifier = Modifier) {
        Canvas(modifier = modifier) {
            try {
                ...
                Log.e("SirKTV_Sidebar", "Search icon draw failed: ${e.stackTraceToString()}")
                drawRoundRect(color = tint, cornerRadius = CornerRadius(3f), size = size)
            }
        }
    }
    ...
    fun Favorites(tint: Color, filled: Boolean, modifier: Modifier = Modifier) {
        Canvas(modifier = modifier) {
            try {
                ...
                Log.e("SirKTV_Sidebar", "Favorites icon draw failed: ${e.stackTraceToString()}")
                drawRoundRect(color = tint, cornerRadius = CornerRadius(3f), size = size)
            }
        }
    }
    ...
    fun Settings(tint: Color, modifier: Modifier = Modifier) {
        Canvas(modifier = modifier) {
            try {
                ...
                Log.e("SirKTV_Sidebar", "Settings icon draw failed: ${e.stackTraceToString()}")
                drawRoundRect(color = tint, cornerRadius = CornerRadius(3f), size = size)
            }
        }
    }
    ...
    fun Person(tint: Color, modifier: Modifier = Modifier) {
        Canvas(modifier = modifier) {
            try {
                ...
                Log.e("SirKTV_Sidebar", "Person icon draw failed: ${e.stackTraceToString()}")
                drawCircle(color = tint, radius = size.minDimension / 3f)
            }
        }
    }
}
